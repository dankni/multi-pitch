...
describe('routeLength for the widget top-of-route estimate', () => {
    it('vertical routes carry their length; traverse routes carry 0', async () => {
        const urls = [];
        mockAxios.get = (url) => { urls.push(url); return Promise.resolve(okResponse); };
        const [vertical, traverse] = await getWeather({ climbs: [
            { id: 6, status: 'publish', geoLocation: '46.503278,11.802140', length: 555 },
            { id: 42, status: 'publish', geoLocation: '46.499111,11.808021', length: 370, traverse: 1 }
        ]});
        expect(vertical.routeLength).to.eql(555);
        expect(traverse.routeLength).to.eql(0); // length covers distance, not height
        expect(urls.some(u => u.includes('elevation='))).to.eql(false); // no extra model calls at all
    });
});
